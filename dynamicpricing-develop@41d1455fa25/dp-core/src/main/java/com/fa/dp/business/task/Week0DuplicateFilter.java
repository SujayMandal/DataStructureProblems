...
@Named
@Scope("prototype")
@CommandDescription(name = "week0DuplicateFilter")
public class Week0DuplicateFilter extends AbstractCommand {
	...
	public void execute(Object data) throws SystemException {
		LOGGER.info("Week0DuplicateFilter -> processTask started.");
		...
		try {
			if (checkData(data, DPProcessParamEntryInfo.class)){
				...
				dpProcessFilterDelegate.filterOnDuplicates(dpProcessParamEntryInfo);
			}
		...
		log.info("Time taken for Week0DuplicateFilter : " + (DateTime.now().getMillis() - startTime) + "ms");
		LOGGER.info("Week0DuplicateFilter -> processTask ended.");
	}
}
