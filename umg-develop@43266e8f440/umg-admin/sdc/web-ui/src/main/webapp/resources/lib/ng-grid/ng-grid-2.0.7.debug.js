...
var ngGrid = function ($scope, options, sortService, domUtilityService, $filter, $templateCache, $utils, $timeout, $parse, $http, $q) {
    var defaults = {
        ...
        */
        ...
        /* included addtional configurable parameter, to show column menu
         * 
         * kabiju.*/
    ...
    self.initTemplates = function() {
        var templates = ['rowTemplate', 'aggregateTemplate', 'headerRowTemplate', 'checkboxCellTemplate', 'checkboxHeaderTemplate', 'menuTemplate', 'footerTemplate','columnMenuTemplate'];
    ...
    $scope.showMenuForEachColumn = self.config.showMenuForEachColumn;
...
ngGridDirectives.directive('ngColumnMenu', ['$compile', '$templateCache', function ($compile, $templateCache) {
    var ngGridMenu = {
        scope: false,
        compile: function () {
            return {
                pre: function ($scope, iElement) {
                    if (iElement.children().length === 0) {
                        iElement.append($compile($templateCache.get($scope.gridId + 'columnMenuTemplate.html'))($scope));
                    }
                }
            };
        }
...
ngGridDirectives.directive('ngShowHideMenu', ['$compile', '$templateCache', function ($compile, $templateCache) {
    return {
    	restrict: 'A',
...
angular.module("ngGrid").run(["$templateCache", function($templateCache) {
  ...
  $templateCache.put("gridTemplate.html",
    ...
    "        <div ng-style=\"rowStyle(row)\" ng-dblclick=\"onDblClickRow(row)\" ng-repeat=\"row in renderedRows\" ng-click=\"row.toggleSelected($event)\" ng-class=\"row.alternatingRowClass()\" ng-row></div>" +
    "    </div>" +
    "</div>" +
    "<div ng-grid-footer></div>" +
    ""
  );
//onDblClick=\"alert('dblclick')\"
  ...
  $templateCache.put("headerRowTemplate.html",
    ...
    "    <div ng-column-menu ng-show=\"showMenuForEachColumn\" ></div>" +	
    "</div>"
  );
  ...
  $templateCache.put("columnMenuTemplate.html",
		    "<div ng-show=\"showMenuForEachColumn\"  ng-show-hide-menu class=\"ngHeaderButton\" >" +
		    "    <div class=\"ngHeaderButtonArrow\"></div>" +
		    "</div>" +
		    "<div style=\"display:none\"   class=\"ngColMenu\">" +
		    "    <div ng-show=\"showFilter\">" +
		    "        <input placeholder=\"{{i18n.ngSearchPlaceHolder}}\" type=\"text\" ng-model=\"filterText\"/>" +
		    "    </div>" +
		    "    <div>" +
