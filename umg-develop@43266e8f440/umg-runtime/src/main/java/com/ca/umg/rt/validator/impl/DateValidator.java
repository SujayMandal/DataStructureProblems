...
public class DateValidator implements TypeValidator{
    ...
    public String validate(Object value) {
        ...
        if(!(value instanceof DateTime)){
            message = String.format("Expected %s but received %s", DataTypes.DATE, value.getClass().getName());
        }
        return message;
    }
    ...
    public String validate(Object value, Map<String, Object> dataTypeProperties, String apiName) {
        ...
        DateTimeFormat.forPattern(pattern).parseDateTime((String)value);
            ...
            message = String.format("Input date is not valid as per the pattern defined for API parameter %s. Received value is %s. Expected pattern is %s", apiName, value, pattern);
        }
        return message;
    }
}
